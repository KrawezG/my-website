...
if (preg_match("![0-9a-z-_]+@[a-z]+[.][a-z]!", $_POST["email"]) &&
	($email_exists == false))
{
    //echo '<div class="sysreq2">почта подходит<br> </div>';
}
else
{
    //echo '<div class="sysreq2">почта не подходит<br></div>';
    $info_is_valid = false;
}
if ($_POST["username"] != NULL &&
	strlen($_POST["username"]) >= 6 &&
 	...
 	($username_exists == false))
{
    //echo '<div class="sysreq1">логин подходит<br></div>';
}
else
{
    //echo '<div class="sysreq1">логин не подходит<br></div>';
    $info_is_valid = false;
}
